...
public class VisitorInformation implements Requestable {
  ...
  public VisitorInformation setPreviousVisitUnixTimestamp( long previousVisitUnixTimestamp ) {
    when( previousVisitUnixTimestamp <= 0 ).throwIllegalArgument( "Timestamp must be > 0, but was "
                                                                 + previousVisitUnixTimestamp );
    addParameter( getRequestKey( VISITOR_PREVIOUS_VISIT ),
                  Long.valueOf( previousVisitUnixTimestamp ) );
    return this;
  }

  public VisitorInformation setFirstVisitUnixTimestamp( long firstVisitUnixTimestamp ) {
    when( firstVisitUnixTimestamp <= 0 ).throwIllegalArgument( "Timestamp must be > 0, but was "
                                                              + firstVisitUnixTimestamp );
    addParameter( getRequestKey( VISITOR_FIRST_VISIT ), Long.valueOf( firstVisitUnixTimestamp ) );
    return this;
  }
