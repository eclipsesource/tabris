...
public class PiwikTrackerTest {
  ...
  public void testSendsPageView() {
    ...
    assertEquals( "foo", actionCaptor.getValue().getParameters().get( getRequestKey( RequestKeys.ACTION_URL ) ) );
  }
  ...
  public void testSendsSearchAction() {
    ...
    assertVisitorInformation( visitorCaptor.getValue() );
    assertEquals( "foo", actionCaptor.getValue().getParameters().get( getRequestKey( RequestKeys.ACTION_URL ) ) );
    assertEquals( "query", actionCaptor.getValue().getParameters().get( getRequestKey( RequestKeys.ACTION_SEARCH ) ) );
  }

  private void assertVisitorInformation( VisitorInformation visitorInformation ) {
    assertEquals( "100x200", visitorInformation.getParameters().get( getRequestKey( RequestKeys.VISITOR_RESOLUTION ) ) );
    assertEquals( "clientId", visitorInformation.getParameters().get( getRequestKey( RequestKeys.VISITOR_ID ) ) );
    assertEquals( "userAgent", visitorInformation.getParameters().get( getRequestKey( RequestKeys.VISITOR_USER_AGENT_OVERRIDE ) ) );
    assertEquals( "en", visitorInformation.getParameters().get( getRequestKey( RequestKeys.VISITOR_LANGUAGE_OVERRIDE ) ) );
    assertCustomVariables( visitorInformation );
  }

  private void assertCustomVariables( VisitorInformation visitorInformation ) {
    JsonObject customVariables = JsonObject.readFrom( ( String )visitorInformation.getParameters()
  ...
  private void assertAdvancedConfiguration( AdvancedConfiguration configuration ) {
    assertEquals( "ip", configuration.getParameters().get( getRequestKey( RequestKeys.VISITOR_IP_OVERRIDE ) ) );
    assertEquals( fakeTokenAuth, configuration.getParameters().get( getRequestKey( RequestKeys.TOKEN_AUTH ) ) );
  }
